...
- **Parámetros** (`/parametros`): precio combustible, costo operador, objetivos de consumo/rendimiento, datos de mantención, ancho de zanja — editables sin tocar código. También permite crear/eliminar las **pozas** de trabajo, cada una con su meta de metros lineales totales y su altura de corte (usados para calcular el % de avance y m³ en el tablero), y los **operadores** que aparecen como lista desplegable en el formulario.
- **Mantenciones realizadas** (dentro de `/parametros`): registra cada mantención hecha al equipo con la fecha, el horómetro en que se hizo, una descripción opcional y un **PDF de respaldo** opcional (factura, checklist, etc.) que se puede subir, ver y descargar después. El tablero usa automáticamente el horómetro de la mantención más reciente para calcular las horas restantes hasta la próxima y alimentar el gráfico "Mantención — horas restantes", en vez de tener que actualizar ese dato a mano en Parámetros cada vez.
- **Cuentas individuales por operador** (`/registro` y `/login`): cada persona crea su propia cuenta (usuario sin espacios + contraseña de al menos 6 caracteres) usando la **clave de equipo** como código de invitación — sin conocerla no se puede crear una cuenta. Una vez registrado, cada quien entra con su propio usuario y contraseña; la clave de equipo ya no sirve para entrar directo, solo para registrarse. El historial de accesos (quién entró, cuándo en hora de Santiago de Chile, y desde qué IP) queda disponible en `/accesos`.
